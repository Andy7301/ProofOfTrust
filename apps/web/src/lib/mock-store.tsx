...
type MockContextValue = {
  ...
  ready: boolean;
  connect: (simulated: boolean) => void;
  disconnect: () => void;
  createRequest: (input: {
    description: string;
  ...
  repayDebt: (debtId: string, txHash: string) => void;
};
...
export function MockStoreProvider({ children }: { children: ReactNode }) {
  ...
  const connect = useCallback((_simulated: boolean) => {
    const address = SIM_ADDRESS;
  ...
  const repayDebt = useCallback((debtId: string, txHash: string) => {
